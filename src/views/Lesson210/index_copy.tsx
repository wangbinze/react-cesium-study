...
const Lesson210 = () => {
    ...
    const loadModel = () => {
            ...
            objLoader.load('./models/123.obj', (object) => {
                ...
                /*if (object instanceof THREE.Group) {
                    ...
                    if (sharkObject instanceof THREE.Mesh) {
                        ...
                        // shark = new THREE.BufferGeometry().copy(sharkGeometry);
                        sharkGeometry.scale(0.05, 0.05, 0.05);
                        sharkGeometry.translate(-0.5, 0.5, 0.2);
        ...
        sharkLoaded = new Promise((resolve, reject) => {
            gltfLoader.load('./models/uuv.glb', (gltf) => {
                console.log('GLTF loaded:', gltf);
                const object = gltf.scene;
                if (object instanceof THREE.Group) {
                    // 检查加载的对象是否是一个 Group 对象
                    const sharkObject = object.children[0]; // 假设鲨鱼是第一个子对象
                    if (sharkObject instanceof THREE.Mesh) {
                        // 检查子对象是否是一个 Mesh 对象
                        sharkObject.geometry.computeBoundingBox();
                        const boundingBox = sharkObject.geometry.boundingBox;
                        const size = new THREE.Vector3();
                        boundingBox.getSize(size);
                        const maxDimension = Math.max(size.x, size.y, size.z);
                        const scale = 0.5 / maxDimension; // 缩放因子，使得模型最大尺寸为0.5
                        sharkObject.scale.set(scale, scale, scale);
                        sharkObject.rotation.set(-Math.PI / 2, 0, 0); // 旋转模型使其平放
                        sharkObject.position.set(0, 0, 0); // 模型位置
                        shark = sharkObject; // 存储模型对象
                        // @ts-ignore
                        resolve();
                    } else {
                        console.error('Child object is not a Mesh:', sharkObject);
                        reject(new Error('Child object is not a Mesh'));
                    }
                } else {
                    console.error('Loaded object is not a Group:', object);
                    reject(new Error('Loaded object is not a Group'));
                }
            }, undefined, (error) => {
    ...
    const init = (canvas: any) => {
        ...
        Promise.all(loaded).then(() => {
            ...
            const envGeometries = [floorGeometry, ...sharkAll, rock1, rock2];
